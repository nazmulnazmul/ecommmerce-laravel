              ...
              <div id="default-datatable_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4"><div class="row"><div class="col-sm-12 col-md-6"><div class="dataTables_length" id="default-datatable_length"><label>Show <select name="default-datatable_length" aria-controls="default-datatable" class="form-control form-control-sm"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select> entries</label></div></div><div class="col-sm-12 col-md-6"><div id="default-datatable_filter" class="dataTables_filter"><label>Search:<input type="search" class="form-control form-control-sm" placeholder="" aria-controls="default-datatable"></label></div></div></div><div class="row"><div class="col-sm-12"><table id="default-datatable" class="table table-bordered dataTable" role="grid" aria-describedby="default-datatable_info">
                <thead>
                    <tr role="row">
                        ...
                        <th class="sorting" tabindex="0" aria-controls="default-datatable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 225px;">Category Photo</th>
                        <th class="sorting" tabindex="0" aria-controls="default-datatable" rowspan="1" colspan="1" aria-label="Office: activate to sort column ascending" style="width: 101px;">Category Slug</th>
                ...
                <tbody>   
                    ...
                    @if ($allCategores->isNotEmpty())
                        @foreach ($allCategores as $key => $category)
                            
                            <tr role="row" class="odd">
                                <td>{{ $key+1 }}</td>
                                <td class="sorting_1">{{ $category->cat_name }}</td>
                                <td class="sorting_1">
                                    <img src="{{ asset($category->cat_image) }}" class="img-thumbnail" alt="" width="100px" height="70px">
                                </td>
                                <td class="sorting_1">{{ $category->cat_slug }}</td>
                                <td class="sorting_1">
                                    @if ($category->cat_status == 1)
                                        <a href="{{ route('admin.category.active', $category->id) }}" class="btn btn-success waves-effect waves-light m-1">Active</a>
                                    @else
                                        <a href="{{ route('admin.category.inActive', $category->id) }}" class="btn btn-danger waves-effect waves-light m-1">Inactive</a>
                                    @endif
                                </td>
                                <td class="sorting_1">
                                    <a href="{{ route('admin.category.edit', $category->id) }}" class="btn btn-success waves-effect waves-light m-1"> <i class="fa-regular fa-pen-to-square"></i>
                                    <a href="{{ route('admin.category.delete', $category->id) }}" onclick="return confirm('Are You Sure !!')"  class="btn btn-danger waves-effect waves-light m-1"> <i class="fa fa-trash-o"></i>
                                </td>
                            </tr>
                        @endforeach
                    @else
                ...
                <tfoot>
                    <tr><th rowspan="1" colspan="1">#Sl.</th><th rowspan="1" colspan="1">Category Name</th><th rowspan="1" colspan="1">Category Slug</th><th rowspan="1" colspan="1">Status</th><th rowspan="1" colspan="1">Action</th></tr>
                </tfoot>
            </table>
        </div>
    </div>
